...
const processMarkdown = (text: string): string => {
  return text.replace(/\*\*(.*?)\*\*/g, '<strong>$1</strong>').replace(/\*(.*?)\*/g, '<em>$1</em>').replace(/__(.*?)__/g, '<strong>$1</strong>').replace(/_(.*?)_/g, '<em>$1</em>').replace(/`(.*?)`/g, '<code>$1</code>').replace(/~~(.*?)~~/g, '<del>$1</del>').replace(/^### (.*$)/gm, '<h3>$1</h3>').replace(/^## (.*$)/gm, '<h2>$1</h2>').replace(/^# (.*$)/gm, '<h1>$1</h1>').replace(/^\* (.*$)/gm, '<li>$1</li>').replace(/^- (.*$)/gm, '<li>$1</li>').replace(/^(\d+)\. (.*$)/gm, '<li>$1. $2</li>');
};
export const AISidebar: React.FC<AISidebarProps> = ({
  ...
  const handleAddToContent = (content: string) => {
    if (onInsertContent) {
      ...
      const cleanedContent = content.replace(/^(Sure,?\s*here\s*(is|are)\s*(the|your)?\s*.*?[:.]\s*)/i, '').replace(/^(Here\s*(is|are)\s*(the|your)?\s*.*?[:.]\s*)/i, '').replace(/^(I'll\s*.*?[:.]\s*)/i, '').replace(/^(Let me\s*.*?[:.]\s*)/i, '').trim();
  ...
  return <div className="fixed right-0 top-0 w-80 h-full bg-background border-l shadow-lg flex flex-col transition-all duration-300 ease-in-out z-40 animate-slide-in-right\n">
      {/* Header */}
